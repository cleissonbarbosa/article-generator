...
export interface ISpeechOptions extends Pick<SpeechRecognition, "lang" | "continuous" | "interimResults"> {
    resultCallback: (transcript: string) => void
    endCallback: () => void
}

export default function speech(options: ISpeechOptions) : boolean | SpeechRecognition {

    // Verifica se a API é suportada pelo navegador
    if ( ! ( 'webkitSpeechRecognition' in window ) ) { 
        console.log('A API de reconhecimento de fala não é suportada pelo seu navegador');
        return false
    }
