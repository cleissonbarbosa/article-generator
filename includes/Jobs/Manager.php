...
class Manager {
    ...
    public function create( $data ) {
        ...
        if ( ! $job_id ) {
            return new \WP_Error( 'article_gen_job_create_failed', __( 'Failed to create job.', 'article-gen' ) );
        }
         ...
         */
        do_action( 'article_gen_jobs_created', $job_id, $job_data );
    ...
    public function update( array $data, int $job_id ) {
        ...
        if ( ! $updated ) {
            return new \WP_Error( 'article_gen_job_update_failed', __( 'Failed to update job.', 'article-gen' ) );
        }
        ...
        if ( $updated >= 0 ) {
             ...
             * @param array $job_data
             */
            do_action( 'article_gen_jobs_updated', $job_id, $job_data );

            return $job_id;
        }

        return new \WP_Error( 'article_gen_job_update_failed', __( 'Failed to update the job.', 'article-gen' ) );
    }
    ...
    public function delete( $job_ids ) {
        ...
        try {
            ...
            foreach ( $job_ids as $job_id ) {
                 ...
                 * @param int $job_id
                 */
                do_action( 'article_gen_job_deleted', $job_id );
            }
            ...
            return new \WP_Error( 'article-gen-job-delete-error', $e->getMessage() );
        }
    }
}
