...
use ArticleGen\JobPlace\Jobs\Job;
use WP_REST_Request;
...
class JobsController extends RESTController {
    ...
    public function get_items( $request ): ?WP_REST_Response {
        ...
        $jobs = article_generator()->jobs->all( $args );
        foreach ( $jobs as $job ) {
        ...
        $args['count'] = 1;
        $total         = article_generator()->jobs->all( $args );
    ...
    public function get_item( $request ) {
        ...
        $job = article_generator()->jobs->get( $args );

        if ( ! $job ) {
            return new WP_Error( 'job_place_rest_job_not_found', __( 'Job not found. May be job has been deleted or you don\'t have access to that.', 'article-gen' ), [ 'status' => 404 ] );
        }
    ...
    public function create_item( $request ) {
        ...
        // Insert the job.
        $job_id = article_generator()->jobs->create( $prepared_data );
        ...
        $job = article_generator()->jobs->get(
            [
				'key' => 'id',
				'value' => $job_id,
			]
    ...
    public function update_item( $request ) {
        ...
        $job_id = article_generator()->jobs->update( $prepared_data, $job_id );
    ...
    public function delete_items( $request ) {
        if ( ! isset( $request['ids'] ) ) {
            return new WP_Error( 'no_ids', __( 'No job ids found.', 'article-gen' ), [ 'status' => 400 ] );
        }

        $deleted = article_generator()->jobs->delete( $request['ids'] );

        if ( $deleted ) {
            $message = __( 'Jobs deleted successfully.', 'article-gen' );
        ...
        return new WP_Error( 'no_job_deleted', __( 'No job deleted. Job has already been deleted. Please try again.', 'article-gen' ), [ 'status' => 400 ] );
    }
    ...
    public function get_item_schema() {
        ...
        $schema = [
            ...
            'properties' => [
                'id' => [
                    'description' => __( 'ID of the job', 'article-gen' ),
                    'type'        => 'integer',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'title' => [
                    'description' => __( 'Job title', 'article-gen' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'required'    => true,
                    'minLength'   => 1,
                ...
                'slug' => [
                    'description' => __( 'Job slug', 'article-gen' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'minLength'   => 1,
                    'arg_options' => [
                ...
                'description' => [
                    'description' => __( 'Job description', 'article-gen' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'required'    => true,
                    'minLength'   => 1,
                ...
                'job_type_id' => [
                    'description' => __( 'Job type', 'article-gen' ),
                    'type'        => 'integer',
                    'context'     => [ 'view', 'edit' ],
                    'required'    => true,
                    'arg_options' => [
                ...
                'is_active' => [
                    'description' => __( 'Job status', 'article-gen' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'required'    => true,
                    'arg_options' => [
    ...
    public function sanitize_job_slug( $slug, $request ) {
        ...
        $total_found = article_generator()->jobs->all( $args );

        if ( $total_found > 0 ) {
            return new WP_Error(
                'job_place_rest_slug_exists', __( 'Job slug already exists.', 'article-gen' ), [
					'status' => 400,
				]
            );
        }
    ...
    public function generate_unique_slug( WP_REST_Request $request ) {
        ...
        if ( empty( $slug ) ) {
            ...
            if ( empty( $request['id'] ) ) {
                $existing_job = article_generator()->jobs->get(
                    [
						'key' => 'slug',
						'value' => $slug,
					]
